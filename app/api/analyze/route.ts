...
export async function POST(request: Request) {
  try {
    ...
    const response = await ai.models.generateContent({
      ...
      contents: [
        filePart,
        `당신은 뜨개질 도안 전문 분석가입니다. 솜씨 좋게 도안 전체를 샅샅이 뒤져서 '원작 실 이름'과 '실 성분(혼용률, 예: 울 80% 나일론 20%)'을 찾아내세요. 
 도안 구석이나 세탁 라벨 안내 문구에 숨어있는 성분 정보가 있다면 절대로 놓치지 말고 추출해 주어야 합니다. 아래 JSON 구조로 응답해주세요.
